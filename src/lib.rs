...
use llvm_sys::analysis::{LLVMVerifyModule, LLVMVerifyFunction, LLVMVerifierFailureAction};
use llvm_sys::target::*;
...
use enzyme_sys::{createEmptyTypeAnalysis, AutoDiff};
...
fn compile_rs_to_bc(entry_file: &PathBuf, out_file: &PathBuf) {
    let rustc_path = utils::get_local_llvm_build_path().join("..").join("stage2").join("bin").join("rustc");
    let mut cmd = process::Command::new(rustc_path);
    cmd.args(&["--emit=llvm-bc", "-O", &entry_file.to_str().unwrap(), "-g", "-o", &out_file.to_str().unwrap()]);
...
unsafe fn read_bc(path: &Path) -> (LLVMModuleRef, LLVMContextRef) {
    ...
    assert_eq!(LLVMCreateMemoryBufferWithContentsOfFile(path.as_ptr(), &mut memory_buf, &mut msg), 0, "could not read in!");
...
unsafe fn load_primary_functions(module: LLVMModuleRef, fnc_names: Vec<String>) -> Vec<LLVMValueRef> {
    ...
    assert_eq!(functions.len(), fnc_names.len(), "load_llvm: couldn't find all functions!");
    functions
}


unsafe fn generate_grad_function(context: LLVMContextRef, mut functions: Vec<LLVMValueRef>)
    -> Vec<LLVMValueRef>{
    ...
    for &mut fnc in functions.iter_mut() {
        let grad_func: LLVMValueRef = auto_diff.create_primal_and_gradient(
            ...
            enzyme_sys::CDIFFE_TYPE::DFT_OUT_DIFF) as LLVMValueRef;
    ...
    assert_eq!(grad_fncs.len(), functions.len(), "failed generating all gradient functions!");
    grad_fncs
}
...
unsafe fn extract_return_type(module: LLVMModuleRef, context: LLVMContextRef, fnc: LLVMValueRef, 
                              u_type: LLVMTypeRef, f_type: LLVMTypeRef, fnc_name: String) -> LLVMValueRef {
    ...
    let new_fnc: LLVMValueRef = LLVMAddFunction(module, c_outer_fnc_name.as_ptr(), LLVMGetElementType(u_type) as LLVMTypeRef);
    ...
    let struct_ret = LLVMBuildCall(builder, fnc, fnc_args.as_mut_ptr(), param_num, c_inner_fnc_name.as_ptr());
    ...
    assert!(LLVMVerifyFunction(new_fnc, LLVMVerifierFailureAction::LLVMAbortProcessAction) == 0, 
            "Could not validate function!");

    let mut msg = ptr::null_mut();
    assert!(LLVMVerifyModule(module, LLVMVerifierFailureAction::LLVMReturnStatusAction, &mut msg) == 0, 
            "Could not validate! {:?}", CStr::from_ptr(msg).to_str().unwrap());
    LLVMDisposeMessage(msg);
...
unsafe fn remove_U_symbols(module: LLVMModuleRef, context: LLVMContextRef, grad_functions: &mut [LLVMValueRef], primary_fnc_names: Vec<String>) {
    for i in 0..grad_functions.len() {
        ...
        assert_ne!(u_fnc as usize, 0, "couldn't get undef symbol {}", new_fnc_name);
        ...
        if u_type != f_type {
            ...
            if LLVMCountStructElementTypes(f_return_type) != 1 {
                panic!("Return struct contains more than one element. u: {:?}, f: {:?}", u_type_string, f_type_string);
            } 
            grad_functions[i] = extract_return_type(module, context, grad_functions[i], u_type, f_type, new_fnc_name.clone());
        }
        
        // Clean up
        LLVMReplaceAllUsesWith(u_fnc, grad_functions[i]);
        LLVMDeleteFunction(u_fnc);
        LLVMSetValueName2(grad_functions[i], c_fnc_name.as_ptr(), new_fnc_name.len() as usize);
    }
}
...
unsafe fn dumb_module_to_obj(module: LLVMModuleRef, context : LLVMContextRef, out_obj: &Path) {
        ...
        let mut msg = ptr::null_mut();
        let c_out_obj = CString::new(out_obj.to_str().unwrap().to_owned()).unwrap().into_raw();
        assert!(LLVMTargetMachineEmitToFile(target_machine, module, c_out_obj, LLVMCodeGenFileType::LLVMObjectFile, &mut msg) == 0, 
                "filename: {:?}, error: {:?}", out_obj, CStr::from_ptr(msg).to_str().unwrap());
        LLVMDisposeMessage(msg);
...
unsafe fn localize_all_symbols(module: LLVMModuleRef) {
    ...
    if symbol == last_symbol { panic!("Found no symbols in module"); }
...
unsafe fn globalize_grad_symbols(module: LLVMModuleRef, primary_fnc_names: Vec<String>) {
    for primary_name in primary_fnc_names {
        ...
        assert_ne!(grad_fnc as usize, 0, "couldn't find function {}", grad_fnc_name);
...
pub fn build<T: AsRef<Path>>(entry_file: T, primary_fnc_names: Vec<String>) {
    ...
    let out_obj = entry_path.clone().with_file_name("result").with_extension("o");
    let out_bc  = entry_path.clone().with_file_name("result").with_extension("bc");
